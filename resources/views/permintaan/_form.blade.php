...
<form method="POST" action="{{ $isEdit ? route('permintaan.update', $permintaan) : route('permintaan.store') }}" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <input type="text" name="judul" value="{{ old('judul', $isEdit ? $permintaan->judul : '') }}" class="form-control @error('judul') is-invalid @enderror" placeholder="mis. Permintaan Gurita Ekspor - PT Sumber Laut Jaya">
        @error('judul')
            ...
            <select name="komoditi_id" id="komoditi_id" class="form-control select2 @error('komoditi_id') is-invalid @enderror" @unless ($isEdit) data-placeholder="-- Pilih Komoditi --" @endunless>
                ...
                @foreach ($komoditiList as $kategori => $daftar)
                    <optgroup label="{{ $kategori ?? 'Lainnya' }}">
                        @foreach ($daftar as $k)
                            <option value="{{ $k->id }}" @selected(old('komoditi_id', $isEdit ? $permintaan->komoditi_id : null) == $k->id)>{{ $k->nama }}</option>
                        @endforeach
                    </optgroup>
                @endforeach
            </select>
    ...
    @role('Pusat|Admin')
        <div class="card card-body bg-light mb-3">
            <h4 class="mb-3">Indikator Prioritas @unless ($isEdit)
                    (khusus tim Pusat)
                @endunless
            </h4>
            <div class="row">
                ...
                <div class="col-md-6 form-group">
                    ...
                    <input type="text" name="prioritas_tag" value="{{ old('prioritas_tag', $isEdit ? $permintaan->prioritas_tag : '') }}" class="form-control" placeholder="mis. Urgent - buyer nunggu 3 hari">
                </div>
            </div>
        </div>
    @endrole
    ...
    <div class="card card-body bg-light mb-3">
        ...
        <x-dropzone-gallery name="foto_gallery" context="foto" :existing-media="$isEdit ? $permintaan->getMedia('foto') : null" :upload-route="route('mediaTemp.upload')" :delete-temp-route="route('mediaTemp.delete')" :delete-existing-url-base="$isEdit ? url('/permintaan/' . $permintaan->id . '/media') : null" accepted-files="image/jpeg,image/png,image/webp" />
    ...
    <div class="card card-body bg-light mb-3">
        ...
        <input type="file" name="video[]" class="filepond" multiple accept="video/mp4,video/quicktime,video/webm" data-max-files="2" @if ($isEdit) data-existing-files='{{ $permintaan->getMedia('video')->map(fn($m) => ['id' => $m->id, 'url' => $m->getUrl(), 'name' => $m->file_name, 'size' => $m->size])->values()->toJson() }}'
                data-delete-existing-url-base="{{ url('/permintaan/' . $permintaan->id . '/media') }}" @endif>
