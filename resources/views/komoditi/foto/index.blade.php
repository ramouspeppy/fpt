            ...
            <div class="card">
                ...
                <div class="card-body text-center">
                    ...
                    @if ($komoditi->fotoUtama())
                        <img src="{{ $komoditi->fotoUtama()->getUrl('thumb') }}" alt="Foto {{ $komoditi->nama }}" class="img-fluid rounded border mb-3" style="max-height: 260px;">
                        <form method="POST" action="{{ route('komoditi.foto.destroy', $komoditi) }}" onsubmit="return confirm('Hapus foto komoditi ini?')">
                            @csrf
                            @method('DELETE')
                            <button class="btn btn-outline-danger btn-sm"><i class="fas fa-trash"></i> Hapus Foto</button>
                        </form>
                    @else
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('komoditi.foto.store', $komoditi) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="file" name="foto" class="filepond-preview @error('foto') is-invalid @enderror" accept="image/jpeg,image/png,image/webp" data-aspect-ratio="1:1" data-preview-height="170">
                        ...
                        <a href="{{ route('komoditi.index') }}" class="btn btn-secondary">Kembali</a>
                    </form>
                </div>
            </div>
        </div>
